...
import { Sparkles, Terminal, Layers, Cpu, ShoppingBag, BarChart3 } from "lucide-react";
import { Badge } from "@/src/components/ui/badge";
import { Section } from "@/src/components/layout/Section";
import { motion, useMotionValue, useSpring } from "framer-motion";

const DOMAIN_CHIPS = [
  { label: "Fintech & SaaS", icon: Terminal },
  { label: "AI Platforms", icon: Cpu },
  { label: "Design Systems", icon: Layers },
  { label: "E-Commerce", icon: ShoppingBag },
  { label: "High-Scale Analytics", icon: BarChart3 },
];

// Interactive Micro-Chip with Spring Physics
function DomainChip({ label, icon: Icon }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <motion.p 
          variants={heroChildVariants}
          className="mt-4 sm:mt-5 text-sm sm:text-base lg:text-lg text-muted-foreground leading-relaxed max-w-2xl font-normal"
        >
          Explore a curated selection of web platforms, digital systems, and custom software
          engineered to drive measurable business impact.
        </motion.p>

        {/* Interactive Domain Capabilities Dock */}
        <motion.div 
          variants={heroChildVariants}
          className="mt-8 sm:mt-10 flex flex-wrap items-center justify-center gap-2.5 sm:gap-3"
        >
          {DOMAIN_CHIPS.map((chip) => (
            <DomainChip key={chip.label} {...chip} />
          ))}
        </motion.div>
