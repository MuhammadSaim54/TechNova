...
import { Sparkles, Clock, ShieldCheck, FileCheck, Zap } from "lucide-react";
import { Badge } from "@/src/components/ui/badge";
import { Section } from "@/src/components/layout/Section";
import { motion, useMotionValue, useSpring } from "framer-motion";

const COMM_SIGNALS = [
  { label: "< 24h Response", icon: Clock },
  { label: "NDA Guaranteed", icon: ShieldCheck },
  { label: "Technical Scoping", icon: FileCheck },
  { label: "Direct Senior Squad", icon: Zap },
];

// Interactive Micro-Chip with Spring Physics
function SignalChip({ label, icon: Icon }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Have an ambitious project in mind? Reach out to our engineering and design squad,
          and let&apos;s architect and launch a digital solution that scales.
        </motion.p>

        {/* Interactive Signal Capabilities Dock */}
        <motion.div 
          variants={heroChildVariants}
          className="mt-8 sm:mt-10 flex flex-wrap items-center justify-center gap-2.5 sm:gap-3"
        >
          {COMM_SIGNALS.map((signal) => (
